...
//Ship constructor
Ship::Ship(float x1, float x2, float x3){
...
void Ship::update_state(){  //call bullet_s update state from here.
  ...
  // Wrap the ship position when at the boundary
    ...
    // 1.1 because the if statements were contradicting each other.
    if(state.cur_location.y <= -1.1){
        state.cur_location.y = 1;
    }
    ...
    if(state.move_down){
        state.cur_location.y -= 0.01;
        state.modelview = Translate(state.cur_location.x, state.cur_location.y, 0.0);
        std::cout<< state.cur_location << std::endl;
    }
    else{
        state.acceleration = 0;
        state.velocity = state.velocity * 0.98;
...
Paddle::Paddle(){
    ...
    //Set up initial state here
    // axis are different from the ship.
      paddle_vert[0] = vec2(-0.99, -1.0);
      paddle_vert[1] = vec2(-0.93, -1.0);
      paddle_vert[2] = vec2(-0.93, -1.1);
      size_t paddle_vert_bytes = 6*sizeof(vec2);
      
      paddle_color[0] = vec3( 0.0, 0.0, 1.0);
      paddle_color[1] = vec3( 0.0, 0.0, 1.0);
      paddle_color[2] = vec3( 0.0, 0.0, 1.0);
      size_t paddle_color_bytes = 6*sizeof(vec3);
    
};

//Called everytime an animation tick happens
void Paddle::update_state(){
    state.paddle_acceleration = 0.01;
    state.paddle_pointing = vec2(1,0);

  vec2 old_velocity = state.paddle_velocity;
    
  state.paddle_velocity = old_velocity + (state.paddle_acceleration * 0.03);
    
    vec2 old_position = state.paddle_cur_location;
    
    state.paddle_cur_location = old_position + (state.paddle_velocity * 0.03);
  // Wrap the ship position when at the boundary
    if(state.paddle_cur_location.x >= 1){
        state.paddle_cur_location.x = -1;
    }
    // When the ship is not yet at the top of the screen.
    state.modelview = Translate(state.paddle_cur_location.x, state.paddle_cur_location.y, 0.0);
    
    if(!state.thruster_on){
        state.paddle_acceleration = state.paddle_pointing;
//        state.modelview *= RotateZ(-10);
    }
    else{
        state.paddle_acceleration = 0;
        state.paddle_velocity = state.paddle_velocity * 0.98;
    }
}
...
void Paddle::gl_init(){
  ...
  //Create GPU buffer to hold vertices and color
  glBufferData( GL_ARRAY_BUFFER, sizeof(paddle_vert) + sizeof(paddle_color), NULL, GL_STATIC_DRAW );
  //First part of array holds vertices
  glBufferSubData( GL_ARRAY_BUFFER, 0, sizeof(paddle_vert), paddle_vert );
  //Second part of array hold colors (offset by sizeof(triangle))
  glBufferSubData( GL_ARRAY_BUFFER, sizeof(paddle_vert), sizeof(paddle_color), paddle_color );
  
  glEnableVertexAttribArray(GLvars.vpos_location);
  glEnableVertexAttribArray(GLvars.vcolor_location );
  
  glVertexAttribPointer( GLvars.vpos_location, 2, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(0) );
  glVertexAttribPointer( GLvars.vcolor_location, 3, GL_FLOAT, GL_FALSE, 0, BUFFER_OFFSET(sizeof(paddle_vert)) );
...
void Paddle::draw(mat4 proj){
  ...
  //Draw something
    ...
    for(int i = 0; i < 7; i++){
        paddle_color[i] = (vec3(0.0, 1.0, 1.0));
    }
    glEnd();
...
////Paddle constructor
//Paddle::Paddle(){
////    state_paddle.ship_system = new Bullet_System(this);
//    GLfloat red = 0.0f;
//
//    state_paddle.pointing = vec2(0,1);
//  //Set up initial state here
//    paddle_vert[0] = vec2(-0.99,  0.0);
//    paddle_vert[1] = vec2(-0.93,  -0.05);
//    paddle_vert[2] = vec2(-0.93,  0.05);
//    size_t paddle_vert_bytes = 6*sizeof(vec2);
//
//    paddle_color[0] = vec3( 0.0, 0.0, 1.0);
//    paddle_color[1] = vec3( 0.0, 0.0, 1.0);
...
//
////Called everytime an animation tick happens
//void Paddle::update_state(){  //call bullet_s update state from here.
//    state_paddle.ship_system -> update();
//
//    pew_pew();
//
//  // Things to do:
...
//  //Force is in the direction the ship is pointing
//  //Clamp acceleration at some maximum value
//    vec2 old_velocity = state_paddle.velocity_paddle;
//    state_paddle.velocity_paddle = old_velocity + (state_paddle.acceleration_paddle * 0.03);
//
//  // clamp velocity at a maximum value
//  // Dampen the velocity at every timestep to lessen intertia
//    vec2 old_position = state_paddle.cur_location;
//
//    state_paddle.cur_location = old_position + (state_paddle.velocity_paddle * 0.03);
//  // Wrap the ship position when at the boundary
//    if(state_paddle.cur_location.y >= 1){
//        state_paddle.cur_location.y = -1;
//    }
//    // When the ship is not yet at the top of the screen.
//    state_paddle.modelview_paddle = Translate(state_paddle.cur_location.x, state_paddle.cur_location.y, 0.0);
//
//    // When the thruster is on.
//    if(state_paddle.thruster_on_paddle){
//        state_paddle.acceleration_paddle = state_paddle.pointing;
//
//    }
//    else{
//        state_paddle.acceleration_paddle = 0;
//        state_paddle.velocity_paddle = state_paddle.velocity_paddle * 0.98;
//    }
//
//}
//// For firing bullets.
//void Paddle::pew_pew(){
//    state_paddle.ship_system -> emit();
//}
//
////Initialize the gl state and variables
//void Paddle::gl_init(){
...
//
//  //If you have a modelview matrix, pass it with proj
//    glUniformMatrix4fv( GLvars.M_location, 1, GL_TRUE, proj * state_paddle.modelview_paddle );
//
//
//  //Draw something
//    glBindVertexArray( GLvars.vao );
...
//
//// new from Bullet_System.
//    glUseProgram(state_paddle.ship_system->program);
//    glBindVertexArray( state_paddle.ship_system->vao );
//    state_paddle.ship_system->update();
//    glUniformMatrix4fv( state_paddle.ship_system->PM_location, 1, GL_TRUE, proj* state_paddle.modelview_paddle );
//    glDrawArrays(GL_POINTS, 0, state_paddle.ship_system->bullets.size());
//
//    //std::cout<< state.ship_system->bullets.size() << std::endl;
//  if(state_paddle.thruster_on_paddle){
//    // Draw something different if the thruster is on.
//      state_paddle.modelview_paddle *= RotateZ(-10);
//      glClearColor(1.0,0.0,0,0);
//
//      paddle_vert[3] = vec2(0.0,  -0.025);
//      paddle_vert[4] = vec2(0.02,  -0.025);
//      paddle_color[3] = vec3( 1.0, 0.0, 1.0);
//      paddle_color[4] = vec3( 1.0, 0.0, 1.0);
//
//
//  }else{
//      glClearColor(0.0,0.0,0,0);
